...
public class AddDoctorPanel extends JPanel {
    ...
    public AddDoctorPanel() {
        ...
        addLabel("Specialist", x1, y, labelFont); specialistField = addField(x1 + 120, y, fieldFont);
...
// fees validation
        ...
        if(fees <= 0){
             JOptionPane.showMessageDialog(this, "Fees should be a positive Number");
            return;
        }
        ...
        if (!birthDate.matches("\\d{4}-\\d{2}-\\d{2}")) {
            ...
            try {
                ...
                empStmt.setDouble(10, 10000.0);
                empStmt.setString(11, "16");


                empStmt.executeUpdate();

                String insertUser = "INSERT INTO users_tbl (username, password_hash, role) VALUES (?, ?, ?)";
                PreparedStatement userStmt = conn.conn.prepareStatement(insertUser);
                userStmt.setString(1, emailField.getText().trim());
